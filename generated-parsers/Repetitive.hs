...
happyExpList = HappyA# "\x18\x00\x20\x00\x00\x47\x00\x00\x00\x02\x00\x00\x00\x80\x00\x00\x00\x00\xec\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"#

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_calc","Ultraroot","bos","eos","tree","Root","A","Bs","B","C","'a'","'b'","'B'","'c'","%eof"]
        bit_start = st * 17
        bit_end = (st + 1) * 17
...
happyGotoValidArray = HappyA# "\x30\x00\x04\x00\x0e\x00\x00\x04\x00\x00\x00\x01\x00\x00\x18\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"#

{-# NOINLINE happyGotoValid #-}
happyGotoValid st nt = valid
  where bit_nr = nt + st * 13
        valid = readArrayBit happyGotoValidArray bit_nr

happyFragileStateArray :: HappyAddr
happyFragileStateArray = HappyA# "\x00\x00\x00\x00"#
...
happyActOffsets = HappyA# "\x13\x00\x17\x00\xfd\xff\x0a\x00\x14\x00\x00\x00\x0f\x00\x00\x00\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"#

happyGotoOffsets :: HappyAddr
happyGotoOffsets = HappyA# "\x12\x00\x03\x00\x0c\x00\x00\x00\x09\x00\x00\x00\x02\x00\x00\x00\xfe\xff\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"#

happyDefActions :: HappyAddr
happyDefActions = HappyA# "\xfd\xff\x00\x00\xf8\xff\x00\x00\xfc\xff\xfb\xff\xf6\xff\xf9\xff\xf2\xff\xfe\xff\xf7\xff\xfa\xff\xf5\xff\xf4\xff\xf3\xff"#

happyCheck :: HappyAddr
happyCheck = HappyA# ""#

happyTable :: HappyAddr
happyTable = HappyA# ""#
...
happyReduce_8 am fragile = happySpecReduce_2  am 6# (happyReduction_8 fragile)
happyReduction_8 fragile p2@(Node (Val {here = (HappyAbsSyn11  happy_var_2)}) _)
	p1@(Node (Val {here = (HappyAbsSyn10  happy_var_1)}) _)
	 =  mkNode (HappyAbsSyn10
		 (happy_var_2:happy_var_1
	)) (Just 7) fragile [p1,p2]
happyReduction_8 fragile _ _  = notHappyAtAll 

happyReduce_9 am fragile = happySpecReduce_0  am 6# (happyReduction_9 fragile)
happyReduction_9 fragile  =  mkNode (HappyAbsSyn10
		 ([]
	)) (Just 7) fragile []
...
happyReduce_13 am fragile = happySpecReduce_0  am 8# (happyReduction_13 fragile)
happyReduction_13 fragile  =  mkNode (HappyAbsSyn12
		 (()
	)) (Just 9) fragile []

happyNewToken verifying action sts stk [] =
	happyDoAction NotVerifying 14# (mkTokensNode [Tok 14# notHappyAtAll]) action sts stk []
...
happyNewToken verifying action sts stk (t:ts) =
	...
	case getTerminals t of {
	  ...
	  (Tok _ tk:tks) ->
	    case tk of {
		TokenA -> cont 10# (setTerminals t (Tok 10# tk:tks)) ((setTerminals t tks):ts);
		TokenBL -> cont 11# (setTerminals t (Tok 11# tk:tks)) ((setTerminals t tks):ts);
		TokenBU -> cont 12# (setTerminals t (Tok 12# tk:tks)) ((setTerminals t tks):ts);
		TokenC -> cont 13# (setTerminals t (Tok 13# tk:tks)) ((setTerminals t tks):ts);
		_ -> happyError' ((t:ts), [])
		};
